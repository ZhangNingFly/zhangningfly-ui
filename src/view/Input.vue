<template>
  <div class="content">
    ...
    <Block-code title="基础用法" :code="code" height="280px">
      <template #source>
      ...
      <template #description>
        <p style="font-size: 14px;color: #5e6d82">
          通过v-model绑定input框输入值，有值时样式不发生改变
        </p>
      </template>
    </Block-code>

     <Block-code title="禁用状态" :code="code1" height="280px">
      <template #source>
        <sxd-input 
        v-model="inputVal" 
        :disabled="true"
    ...
    <!--<sxd-switch a="from father">
      <template v-slot:head>
        <button>from head slot</button>
      </template>
      <template v-slot:body>
        <button>from body slot</button>
      </template>
      <template v-slot="slotProps">
        <button @click="slot(slotProps)">from foot slot{{slotProps.color}}</button>
      </template>
    </sxd-switch>-->
  </div>
</template>
...
export default {
  data () {
    return {
       ...
       code:
           `
    <input v-model="inputVal" label="请输入"/>
